# 🍷 와인 품질 회귀 분석 (Wine Quality Regression Analysis)
> AI & Data Science 수업 – 회귀분석 확장 실습

## 1. 분석보고서 개요

이 분석보고서는 와인 화학적 특성과 와인 유형(type)을 활용하여  
와인의 품질 점수(quality)를 예측하는 회귀 모델을 구축하는 것을 목표로 한다.

수업 과제로 수행했던 탐색적 데이터 분석(`winequ-ana.ipynb`)에 그치지 않고,  
회귀 모델 구축 및 성능 평가까지 분석을 확장해서
...
### (4) 검증 데이터  / 테스트 데이터 전처리

- Train 데이터에서 학습된(fit) 기준(bounds(이상치처리), encoder(원핫인코딩), scaler(수치형 변수 스케일링))을  
  검증 / 테스트 데이터에 **동일하게 적용(transform)**
...
성능 비교 결과,  
Linesar Regression이 가장 좋은 성능을 보였으나, Ridge 모델과 거의 동일한 성능을 보였다.

데이터에 다중공선성이 높은 변수(`density(19.034597833404764)`, `residual sugar(8.713409485251605)`)가 존재한다는 점을 고려하여,  
계수의 안정성을 확보할 수 있는 릿지를 최종 모델로 선택하였다.  
